...
class TokenManager(cacheManager: CacheManager) {
    ...
    fun generate(authentication: OAuth2AuthenticationToken): String {
        ...
        val email = getUserName(authentication.principal.attributes, authentication.authorizedClientRegistrationId)
    ...
    fun getUserName(attributes: Map<String, Any>, registrationId: String): String {
        return when (registrationId) {
            "github" -> attributes["login"] as String
            else -> attributes["email"] as String
        }
    }
}